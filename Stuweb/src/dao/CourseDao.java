...
public class CourseDao extends BaseDao {
	public boolean addCourse(Course course){
		String sql = "insert into s_course values(null,'"+course.getName()+"',"+course.getTeacherId()+",'"+course.getCourseDate()+"','"+course.getStartDate()+"',0,"+course.getMaxNum()+",'"+course.getInfo()+"') ";
		return update(sql);
	}
	...
	public List<Course> getCourseList(Course course,Page page){
		...
		try {
			while(resultSet.next()){
				...
				cl.setStartDate(resultSet.getString("start_date"));
	...
	public boolean editCourse(Course course) {
		...
		String sql = "update s_course set name = '"+course.getName()+"',teacher_id = "+course.getTeacherId()+",course_date = '"+course.getCourseDate()+"',start_date = '"+course.getStartDate()+"',max_num = "+course.getMaxNum()+" ,info = '"+course.getInfo()+ "' where id = " + course.getId();
		return update(sql);
	}
	...
	public List<Course> getCourse(String ids){
		...
		try {
			while(query.next()){
				...
				cl.setStartDate(query.getString("start_date"));
	...
	public Course getCourseCid(int id){
		Course ret = null;
		...
		try {
			while(query.next()){
				...
				ret.setName(query.getString("name"));
				ret.setTeacherId(query.getInt("teacher_id"));
				ret.setCourseDate(query.getString("course_date"));
				ret.setStartDate(query.getString("start_date"));
				ret.setSelectedNum(query.getInt("selected_num"));
				ret.setMaxNum(query.getInt("max_num"));
				ret.setInfo(query.getString("info"));
				return ret;
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
	...
	public List<Course> getCourseTid(int id){
		List<Course> ret = new ArrayList<Course>();
		String sql = "select * from s_course where teacher_id = " + id;
		ResultSet query = query(sql);
		try {
			while(query.next()){
				Course cl = new Course();
				...
				cl.setTeacherId(query.getInt("teacher_id"));
				cl.setCourseDate(query.getString("course_date"));
				cl.setCourseDate(query.getString("start_date"));
				cl.setSelectedNum(query.getInt("selected_num"));
				cl.setMaxNum(query.getInt("max_num"));
				cl.setInfo(query.getString("info"));
				ret.add(cl);
	...
	public Course getCourseCdate(String cdate){
		Course ret = null;
		String sql = "select * from s_course where course_date = (" + cdate + ")";
		ResultSet query = query(sql);
		try {
			while(query.next()){
				ret = new Course();
				...
				ret.setTeacherId(query.getInt("teacher_id"));
				ret.setCourseDate(query.getString("course_date"));
				ret.setCourseDate(query.getString("start_date"));
				ret.setSelectedNum(query.getInt("selected_num"));
				ret.setMaxNum(query.getInt("max_num"));
				ret.setInfo(query.getString("info"));
				return ret;
	...
	public List<Course> getCourseC(String cdates){
		List<Course> ret = new ArrayList<Course>();
		String sql = "select * from s_course where cdate in("+cdates+")";
		ResultSet query = query(sql);
		try {
			while(query.next()){
				Course cl = new Course();
