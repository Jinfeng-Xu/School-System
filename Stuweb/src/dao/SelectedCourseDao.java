...
public class SelectedCourseDao extends BaseDao {
	public List<SelectedCourse> getSelectedCourseList(SelectedCourse selectedCourse,Page page){
		...
		if(selectedCourse.getCount() != 0) {
			sql += " and count = " + selectedCourse.getCount();
		}
		...
		try {
			while(resultSet.next()){
				...
				cl.setCount(resultSet.getInt("count"));
				ret.add(cl);
			}
	...
	public boolean addSelectedCourse(SelectedCourse selectedCourse){
		String sql = "insert into s_selected_course values(null,"+selectedCourse.getStudentId()+","+selectedCourse.getCourseId()+","+selectedCourse.getCount()+")";
		return update(sql);
	}
	/**
	 * 删除所选课程
	...
	public boolean editSelectedCourse(SelectedCourse selectedCourse) {
		// TODO Auto-generated method stub
		String sql = "update s_selected_course set count = "+ selectedCourse.getCount() + " WHERE id = " + selectedCourse.getId();
//		String sql = "update s_selected_course set count = " + selectedCourse.getCount() + "where id = " + selectedCourse.getId();
		return update(sql);
	}	
	/**
	...
	public SelectedCourse getSelectedCourse(int id){
		...
		try {
			if(query.next()){
				...
				ret.setCount(query.getInt("count"));
				return ret;
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
	...
	public SelectedCourse getSelectedCourse2(int id){
		SelectedCourse ret = null;
		String sql = "select * from s_selected_course where course_id = " + id;
		ResultSet query = query(sql);
		try {
			if(query.next()){
				ret = new SelectedCourse();
	...
	public List<SelectedCourse> getSelectedCourseList(int id){
		List<SelectedCourse> ret = new ArrayList<SelectedCourse>();
		String sql = "select * from s_selected_course where student_id = " + id;
		ResultSet resultSet = query(sql);
		try {
			while(resultSet.next()){
				SelectedCourse cl = new SelectedCourse();
