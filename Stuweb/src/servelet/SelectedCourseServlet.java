...
public class SelectedCourseServlet extends HttpServlet {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 7595040223772789063L;
	...
	private void getSelectedCourseList(HttpServletRequest request,
		...
		int studentId = request.getParameter("studentid") == null ? 0 : Integer.parseInt(request.getParameter("studentid").toString());
		int courseId = request.getParameter("courseid") == null ? 0 : Integer.parseInt(request.getParameter("courseid").toString());
		int count = request.getParameter("count") == null ? 0 : Integer.parseInt(request.getParameter("count").toString());
		Integer currentPage = request.getParameter("page") == null ? 1 : Integer.parseInt(request.getParameter("page"));
		Integer pageSize = request.getParameter("rows") == null ? 999 : Integer.parseInt(request.getParameter("rows"));
//		CourseDao courseDao = new CourseDao();
//		int count = courseDao.getCourse(String.valueOf(courseId)).get(0).getCount();
//		getServletContext().setAttribute("status", count);
		SelectedCourse selectedCourse = new SelectedCourse();
		//获取当前登录用户类型
